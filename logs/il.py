...
def sample_sites(lattice_dims, n_samples):
    """
    ...
    # pull from list 0:(lattice_dims-2) without replacement to get possible sites
    idx = np.random.choice(np.prod(lattice_dims), n_samples, replace=False)
    # Relate the list number to its array index
    # answer = np.vstack(np.unravel_index(idx, lattice_dims)).T
    # Shift up and right by one
    return idx

def inspect_sites(lattice_len, nearest_neighbor_distances, barrier_heights, n_samples):
    """
    ...
    sampled_sites = sample_sites(lattice_len, n_samples)
    sampled_barrier_heights = barrier_heights[sampled_sites]
    sampled_NN_distances = nearest_neighbor_distances[sampled_sites]
...
def biased_sample(model_barrier_heights,T):
    ...
    return np.random.choice(np.arange(np.shape(model_barrier_heights)[0]),replace=True, p = exp_E)
...
def predicted_adsorption_energies(training_NN_distances, training_barrier_heights, M, NN_distances, lattice_size):
    """
    ...
    predicted_energies = np.zeros((lattice_size-2)**2)
    predicted_energies = np.zeros(lattice_size)
    training_size = len(training_barrier_heights)
    training_barrier_heights = np.longdouble(training_barrier_heights)

    # for i in range((lattice_size-2)**2):
    for i in range(lattice_size):
        delta = training_NN_distances-NN_distances[i]
...
def importance_learning(barrier_distribution, NN_distances, T, initial_pool_size, n_iterations, plot_every=5, verbose=True):
    """
    ...
    for i in range(n_iterations):
        # sampled_sites_raveled = [np.ravel_multi_index(sampled_sites[x]-1, (lattice_size - 2, lattice_size - 2)) for x in range(len(sampled_sites))]

        # Excluding sites sampled more than once 
        indices_NN = np.unique(sampled_NN_distances,return_index = True, axis = 0)[1]
        sampled_NN_distance_unique  = np.asarray([sampled_NN_distances[index] for index in sorted(indices_NN)])
        ...
        #TRAINING
        M, model_barriers_LOO, residuals = train(sampled_NN_distance_unique, sampled_barrier_heights_unique)

        # Get model predicted barrier heights for all l^2 sites
        
        model_barrier_heights = predicted_adsorption_energies(sampled_NN_distance_unique, sampled_barrier_heights_unique, M, NN_distances, lattice_len) 
        ...
        if verbose:
            ...
            print("NN Distances", NN_distances[test_site])
        ...
        sampled_NN_distances = np.append(sampled_NN_distances, [NN_distances[test_site]], axis=0)
    return IL