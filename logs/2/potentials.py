...
def bare_metal_pot(M,D_MO,r_eq_MO,a_MO,D_M_O,r_eq_M_O,a_M_O,S1,S2,S3,S4):
    """
    Potential for the bare metal site, this includes both the hydroxyl oxygens and both the siloxanes
    Inputs -
        M - coordinates of metal
        D_MO - Dissociation energy for the Morse Potential for the metal-siloxane bond
        r_eq_MO - Equilibrium distance of the Morse Potential for the metal-siloxane bond
        a_MO - a parameter of the Morse Potential for the metal-siloxane bond
        D_M_O - Dissociation energy for the Morse Potential for the metal-oxygen bond
        r_eq_M_O - Equilibrium distance for the Morse Potential for the metal-oxygen bond
        a_M_O - a parameter of the Morse Potential for the metal-oxygen bond
   ...
   # "Solve equation to get lumped"
    ### in the ordered lattice
    #Delta_H_ads = 2*E_M_O + +2*E_M__O + C == 0
    C = -(2*E_MO + +2*E_M_O)
...
# Calculate distance of metal from S1, S2, S3, and S4
    ...
    return morse_potential(r1, r_eq_MO, D_MO, a_MO) + morse_potential(r2, r_eq_M_O, D_M_O, a_M_O) + morse_potential(r3, r_eq_MO, D_MO, a_MO) + morse_potential(r4, r_eq_M_O, D_M_O, a_M_O)

def ads_state_pot(M, D_MO, r_eq_MO, a_MO, D_M_O, r_eq_M_O, a_M_O, S1, S2, S3, S4):
    """
   ...
   # "Solve equation to get lumped"
    ...
    return morse_potential(r1, r_eq_MO, D_MO, a_MO) + morse_potential(r2, r_eq_M_O, D_M_O, a_M_O) + morse_potential(r3, r_eq_MO, D_MO, a_MO) + morse_potential(r4, r_eq_M_O, D_M_O, a_M_O)+ modified_morse(r4, r_eq_M_O, D_M_O, a_M_O)

def opt_bare_metal(D_MO, r_eq_MO, a_MO, D_M_O, r_eq_M_O, a_M_O, S1, S2, S3, S4):
    """
    ...
    M = (S1+S2+S3+S4)/4
    return minimize(bare_metal_pot,M,method='BFGS',args=(D_MO,r_eq_MO,a_MO,D_M_O,r_eq_M_O,a_M_O,S1,S2,S3,S4))

def opt_ads_state(D_MO, r_eq_MO, a_MO, D_M_O, r_eq_M_O, a_M_O, S1, S2, S3, S4):
    """
    ...
    # Generate initial guess for metal position
    M = (S1+S2+S3+S4)/4
    return minimize(ads_state_pot, M, method='BFGS', args=(D_MO, r_eq_MO, a_MO, D_M_O, r_eq_M_O, a_M_O, S1, S2, S3, S4))

def grafting_Energies(D_MO, r_eq_MO, a_MO, D_M_O, r_eq_M_O, a_M_O, graftable_sites, decorated_lattice):
    """
    ...
    unperturbed_opt = opt_bare_metal(D_MO,r_eq_MO,a_MO,D_M_O,r_eq_M_O,a_M_O,S10,S30,S20,S40)
    # C = 70. - unperturbed_opt.fun
    E_MA = 250
    ...
    C = 70 - (unperturbed_opt.fun + 2*trans_free_E_HL  - trans_free_E_ML2)
    ...
    for i in range(np.shape(graftable_sites)[0]):
        ...
        #optimize the bare metal site
        bare_metal = opt_bare_metal(D_MO,r_eq_MO,a_MO,D_M_O,r_eq_M_O,a_M_O,S1,S3,S2,S4)
        ...
        index_del = np.argmax((metal_dist(bare_metal.x[0],bare_metal.x[1],S2),metal_dist(bare_metal.x[0],bare_metal.x[1],S4)))

        # E_MO = Energy of the Cr_O bond = 500 kJ/mol
        E_MO = 500
        ...
        if index_del == 0 :
            ...
            ads_state = opt_ads_state(D_MO,r_eq_MO,a_MO,D_M_O,r_eq_M_O,a_M_O,S1,S4,S3,S2)
            opt_ads_E.append(ads_state.fun)
        if index_del == 1 :
            #delete S4 and return opt energy
            ads_state = opt_ads_state(D_MO,r_eq_MO,a_MO,D_M_O,r_eq_M_O,a_M_O,S1,S2,S3,S4)
            opt_ads_E.append(ads_state.fun)

    return  C + np.asarray(opt_graft_E) + 2*trans_free_E_HL  - trans_free_E_ML2, -E_MA + np.asarray(opt_ads_E)-(np.asarray(opt_graft_E) + trans_free_E_HL)
