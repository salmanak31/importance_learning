...
def histogram(ax1, ax2, barrier_distribution, T, n_bins = 35):
    ...
    avg_E = np.mean(barrier_distribution)
    # Unbiased
    ...
    ax1.plot([avg_E, avg_E], [0, max(y) + max(y)*0.05], linewidth=2, c='k')
    ...
    ax2.plot([avg_E, avg_E], [0, max(y) + max(y)*0.05], linewidth=2, c='k')
...
def plot_trained(ax, model_barriers, true_barriers, confidence_int, initial_pool_size):
    """
    ...
    ax.set_xlabel(r'Model $E_a$, kJ/mol')
    ax.set_ylabel(r'True $E_a$ kJ/mol')
