...
def make_quenched_disorder_lattice(lattice_length, displacement_amt, dist_type, use_seed=False, plot_dist_r=False, lower=None, upper=None, sigma=None, mean=None):
    """
...
def decorate_lattice(lattice, grafting_fraction, OH_fraction, O_fraction):
    ...
    """
    return np.dstack((lattice, np.random.choice([0, 1, 2], size=lattice.shape[0:2], p=[grafting_fraction, OH_fraction, O_fraction])))
...
def locate_grafting_sites(decorated_lattice):
    ...
    outputs: 
       ...
       - (2 x 2 x l) array of sites that share OH or Si-O-Si pairs
    """
    ...
    return np.asarray(graftable_sites), competing_sites
