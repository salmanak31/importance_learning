...
def histogram(ax1, ax2, barrier_distribution, T, n_bins = 35):
    ...
    ax2.plot([site_avg_E, site_avg_E], [0, max(y) + max(y)*0.05], linewidth=2, c='k', label=r'$\langle E_a \rangle_k$')
    # ax2.annotate(r'$\langle E_a \rangle_k$', (site_avg_E + 1, 3 * max(y)/4))
    ax2.set_xlabel(r'$E_a$, kJ/mol')
    ax2.set_ylabel(r'$\tilde{\rho}_w(E_a)$')    
    ax2.set_ylim([0, max(y) + max(y)*0.05])

    return ax1, ax2

def plot_trained(ax, model_barriers, true_barriers, initial_pool_size, confidence_int=0):
    """
    ...
    # Offsets from when the model was not as accurate
    if confidence_int != 0:
    ...
    if initial_pool_size == len(true_barriers):
        # plot initial pool
        ax.plot(true_barriers, model_barriers,'ro', label='Initial pool', c="blue")
    else:
        # plot initial pool
    ...
    ax.set_ylabel(r'$\hat{E}_a$ / (kJ/mol)')
    ax.set_xlabel(r'$E_a$ / (kJ/mol)')
...
def plot_convergence(ax, n_iterations, pred_Ea, sampling_error, true_avg_Ea, bars_every=1):
    ...
    ax.plot([0, n_iterations], [true_avg_Ea, true_avg_Ea], c='k', linestyle='--', label=r'$\langle E_a \rangle_k$')
    ax.set_xlabel("Iteration")
    ax.set_ylabel(r'$\overline{E}_a$, kJ/mol')
...
def plot_residuals(ax, true_barriers, model_barriers, n_bins=40):
    ...
    sns.distplot(residuals, bins=n_bins, color="grey")
